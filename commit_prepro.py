#!/usr/bin/env python
# -*- coding: utf-8 -*-
# File              : commit_prepro.py
# Author            : Sun Fu <cstsunfu@gmail.com>
# Date              : 23.06.2018
# Last Modified Date: 26.08.2018
        ...
        for article in tqdm(source["data"]):
            for para in article["paragraphs"]:
                ...
                for qa in para["qas"]:
                    ...
                    ques_tf = [float(counter_[w]) / float(tf_total) for w in ques_tokens]
...
def build_features(examples, data_type, out_file, word2idx_dict, char2idx_dict, pos2idx_dict, ner2idx_dict, drop_file=None, is_test=False):
    ...
    para_limit = 600 if is_test else 400
    ...
    for example in tqdm(examples):
        ...
        context_tokens.append(example["context_tokens"])
        ...
        for i, match in enumerate(example["ques_match_origin"]) :
            ques_match_origin[i] = 1 if match == True else 0
        for i, match in enumerate(example["ques_match_lower"]) :
            ques_match_lower[i] = 1 if match == True else 0
        for i, match in enumerate(example["ques_match_lemma"]) :
            ques_match_lemma[i] = 1 if match == True else 0
        for i, tf in enumerate(example['ques_tf']) :
            ques_tf[i] = tf

        ques_tokens.append(example["ques_tokens"])
...
if __name__ == '__main__' :
    ...
    train_examples, train_eval = process_file(save_dir + 'train-v2.0.json', "train")
    ...
    with open(save_dir + 'word2id.json', 'r', encoding='utf-8') as f :
        word2id = json.load(f)

    with open(save_dir + 'char2id.json', 'r', encoding='utf-8') as f :
        char2id = json.load(f)

    with open(save_dir + 'pos2id.json', 'r') as f :
        pos2id = json.load(f)

    with open(save_dir + 'ner2id.json', 'r') as f :
        ner2id = json.load(f)
    build_features(train_examples, "train",
                   save_dir + 'train1.json', word2id, char2id, pos2id, ner2id)


    with open(save_dir + 'train_eval1.json', 'w', encoding='utf-8') as f :
